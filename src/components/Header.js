        ...
        <nav className="w-full h-[10vh] bg-slate-100 text-red-700 text-base font-medium">
            <div className="h-full flex justify-between items-center px-6 py-3">
                ...
                {!userData && <div className="hidden md:block">
                    <button onClick={() => setIsLoginModalOpen(true)} className="text-red-700 bg-yellow-500 px-4 py-2 rounded-md mx-2">Login</button>
                    <button onClick={() => setIsRegisterModalOpen(true)} className=" bg-red-700 text-slate-100 px-4 py-2 rounded-md">Register</button>
                </div>}  
                {userData && <div className="hidden md:flex items-center gap-3">
                <button onClick={() => {setUserData(null); localStorage.removeItem("travelAuthToken")}} className="px-4 py-2 rounded-md bg-red-700 text-slate-100 hover:bg-red-800">Log Out</button>
                    </div>}
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden absolute bg-slate-100 w-full p-3">
                    ...
                    {!userData && <div className="my-2">
                        <button onClick={() => {setIsLoginModalOpen(true); toggleMobileMenu()}} className="text-red-700 bg-yellow-500 px-4 py-2 rounded-md">Login</button>
                        <button onClick={() => {setIsRegisterModalOpen(true); toggleMobileMenu()}} className="text-slate-100 bg-red-700 px-4 py-2 rounded-md mx-2">Register</button>
                    </div>}
